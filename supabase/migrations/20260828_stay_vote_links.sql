...
create table if not exists public.stay_poll_options (
  ...
  name text not null check (length(trim(name)) between 1 and 160), details text, price numeric(12,2) check (price is null or price>=0), image_url text, sort_order integer not null default 0
);
...
returns uuid language plpgsql security definer set search_path=public as $$ declare new_poll uuid; begin
  ...
  insert into public.stay_poll_options(poll_id,name,details,price,image_url,sort_order) select id,trim(value->>'name'),nullif(value->>'details',''),nullif(value->>'price','')::numeric,nullif(value->>'image_url',''),coalesce((value->>'sort_order')::int,0) from public.stay_polls p cross join jsonb_array_elements(poll_options) value where p.token=new_poll;
  return new_poll;
end; $$;
